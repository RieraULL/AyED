...
int sumatorio(const vector<int> &V, const unsigned int i,
...
// maximo de un vector
int maximo(const vector<int> &V, const unsigned int i,
           const bool traza = false, const int w = 1)
{
  if (i == 0)
  {
...
int main()
{
  ...
  cout << endl
       << "*** EXPONENCIACIÓN b^e RECURSIVA ***" << endl;
  unsigned int e;
  int b;
  cout << "b ? ", cin >> b;
  cout << "e ? ", cin >> e;
  ...
  cout << endl
       << "*** SUMATORIO SOBRE UN VECTOR ***" << endl;
  int N = 9;
  vector<int> V(N);
  for (int i = 0; i < N; i++)
    V[i] = i + 1;
  ...
  cout << endl
       << "*** ELEMENTO N-ÉSIMO DE LA SUCESIÓN DE FIBONACCI RECURSIVO ***" << endl;
  unsigned int n;
  cout << "n ? ", cin >> n;
  ...
  cout << endl
       << "*** COEFICIENTE BINOMIAL C(n, k) RECURSIVO ***" << endl;
  unsigned int k;
  cout << "n ? ", cin >> n;
  cout << "k ? ", cin >> k;
  assert(k <= n);
